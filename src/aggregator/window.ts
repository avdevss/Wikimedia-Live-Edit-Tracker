...
export function ingest(w: SlidingWindow, member: string, bucketTs: number) {
  if (w.lastBucketTs < 0) {
  ...
  } else if (bucketTs > w.lastBucketTs) {
    // Walk forward one second at a time, expiring whichever bucket just
    // fell out of the 300s window at each step. Capped at WINDOW_SECONDS
    // steps: after a gap that long, every bucket is stale, so there is no
    // point walking further than that.
    ...
    w.lastBucketTs = bucketTs;
  } else if (bucketTs <= w.lastBucketTs - WINDOW_SECONDS) {
